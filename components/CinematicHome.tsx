...
function StudioInterface() {
  ...
  return <div className="studio-interface" aria-hidden="true">
    <div className="model-platform"><div className="studio-mass m1"/><div className="studio-mass m2"/><div className="studio-mass m3"/><div className="facade-lines"/></div>
...
export function CinematicHome(){
  ...
  useEffect(()=>{
    ...
    const ctx=gsap.context(()=>{
      ...
      gsap.utils.toArray<HTMLElement>('.world').forEach((section)=>{
        ...
        gsap.fromTo(objects,{y:(i)=>80+i*60,opacity:0,rotateX:8},{y:(i)=>-30-i*35,opacity:1,rotateX:0,stagger:.08,ease:'none',scrollTrigger:{trigger:section,start:'top 82%',end:'bottom 15%',scrub:1}});
      ...
      portfolio.from('.case-map',{opacity:0,scale:1.25}).to('.case-map',{opacity:.25,scale:1},.8)
        ...
        .to('.case-wire',{opacity:.2},1.4).fromTo('.case-render',{clipPath:'inset(100% 0 0 0)',scale:1.14},{clipPath:'inset(0% 0 0 0)',scale:1,duration:1.4},2.2)
        .from('.case-caption',{opacity:0,y:40},3);
    },root);
    return ()=>ctx.revert();
  },[]);
  return <div ref={root} className="cinematic-home">
    <section className="cinematic-hero">
      ...
      <div className="tower-canvas"><TowerScene/></div>
    ...
    <section id="intelligence" className="world intelligence-world"><span className="chapter-no">01 / 03</span><div className="orb depth"/><IntelligenceInterface/><div className="world-copy depth"><p>STRUCTŪRA INTELLIGENCE</p><h2>See the decision<br/><em>before it exists.</em></h2><p>Property geometry, market context and capital structure—resolved into one legible field.</p><Link href="/intelligence">Explore Intelligence <Arrow/></Link></div></section>
    ...
    <section className="closing"><p>STRUCTŪRA / NEW YORK · GLOBAL</p><h2>Every consequential<br/>decision leaves a <em>structure.</em></h2><div><Link href="/contact">Begin a conversation <Arrow/></Link><Link href="/studio/portfolio">View all studies <Arrow/></Link></div></section>
  </div>
}
